...
public static class MoveTypeToFileTool
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var duplicateDoc = await ExxerFactoringHelpers.FindTypeInSolution(solution, typeName, filePath, newFilePath);
            ...
            rootWithoutType = (CompilationUnitSyntax)Formatter.Format(rootWithoutType, ExxerFactoringHelpers.SharedWorkspace);
            var sourceEncoding = await ExxerFactoringHelpers.GetFileEncodingAsync(filePath, cancellationToken);
            ...
            newRoot = (CompilationUnitSyntax)Formatter.Format(newRoot, ExxerFactoringHelpers.SharedWorkspace);
            if (File.Exists(newFilePath))
                throw new McpException($"Error: File {newFilePath} already exists");
            var newFileEncoding = await ExxerFactoringHelpers.GetFileEncodingAsync(filePath, cancellationToken);
            await File.WriteAllTextAsync(newFilePath, newRoot.ToFullString(), newFileEncoding, cancellationToken);

            if (document != null)
            {
                ExxerFactoringHelpers.AddDocumentToProject(document.Project, newFilePath);
            }
            else
            {
