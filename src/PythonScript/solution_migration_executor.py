...
class EnhancedMigrationExecutor:
    ...
    def _load_resume_state(self):
        ...
        if not self.dry_run and self.resume_log_path.exists():
            ...
            try:
                ...
                self.logger.warning(f"⚠️ Could not load resume state: {e}")
        else:
    ...
    def _check_disk_space(self, dest_path: Path, required_bytes: int) -> bool:
        ...
        try:
            ...
            if free_bytes < required_bytes * 1.1:  # 10% buffer
                self.logger.error(f"❌ Insufficient disk space! Need {required_mb:.1f} MB, have {free_mb:.1f} MB")
                return False
        ...
        while retry_count <= max_retries:
            try:
                ...
                if retry_count <= max_retries:
                    self.logger.warning(f"⚠️ Retry {retry_count}/{max_retries} for {source_path}: {e}")
                    ...
                    self.logger.error(f"❌ Failed to copy {source_path} after {max_retries} retries: {e}")
                    return False
    ...
    def execute_migration(self) -> bool:
        ...
        try:
            ...
            if not source_base.exists():
                self.logger.error(f"❌ Source path does not exist: {source_base}")
                return False
            ...
            self.logger.info(f"   ✅ Successfully processed: {success_count:,} files ({self.copied_size / (1024*1024):.1f} MB)")
    ...
    def _generate_completion_report(self):
        ...
        self.logger.info(f"   ✅ Created: migration_report{mode_suffix}_{timestamp_str}.json")
        self.logger.info(f"   ✅ Created: migration_summary{mode_suffix}_{timestamp_str}.txt")
